...
public :
	...
	constexpr modint &operator/= (const modint &p) noexcept { return ((*this) *= p.inverse()); }
	...
	constexpr bool operator!= (const modint &p) const noexcept { return (x != p.x); }
	constexpr modint inverse () const noexcept { return pow(mod - 2); }
	constexpr modint pow (std::uint_fast64_t exp) const noexcept {
		modint value(1), buff(x);
		while (exp) { if (exp & 1) value *= buff; buff *= buff; exp >>= 1; }
		return value;
	}
	friend std::ostream &operator<< (std::ostream &os, const modint &p) { return (os << p.x); }
	friend std::istream &operator>> (std::istream &is, modint &p) { long long v; is >> v; p = modint(v); return (is); }
};